...
xTrainingOnes = np.matrix([onesList(len(xTraining)), xTraining, np.square(xTraining)]).getT()
...
xTestOnes = np.matrix([onesList(len(xTest)), xTest, np.square(xTest)]).getT()
...
xN = np.matrix([onesList(len(x)) , x, np.square(x)]).getT()
