...
xTrainingOnes = np.matrix([onesList(len(xTraining)), xTraining, np.square(xTraining), np.power(xTraining, 3)]).getT()
...
xTestOnes = np.matrix([onesList(len(xTest)), xTest, np.square(xTest), np.power(xTest, 3)]).getT()
...
xN = np.matrix([onesList(len(x)) , x, np.square(x), np.power(x, 3)]).getT()
